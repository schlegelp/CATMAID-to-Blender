    ...
    def execute(self, context):
        ...
        if self.annotations:
            ...
            if self.annotations.startswith('"') and self.annotations.endswith('"'):
                annotations_to_retrieve = [self.annotations[1:-1]]
            else:
...
class Create_Mesh (Operator):
    ...
    def make_curve_neuron (neuron_name, root_node, nodes_dic, child_list,
        ...
        # We need to store as float as C otherwise complains if int is too large
        # ob['node_ids'] = list(np.array(node_ids).astype(float))
...
class AnimateHistory(Operator):
    ...
    def plot_history(self, neuron):
        ...
        if not all_timestamps:
            self.report({'WARNING'},'Neuron #{} has no timestamps! Skipping...'.format(neuron))
            print('Neuron #{} has no timestamps! Skipping...'.format(neuron))
            return
...
class ColorBySpatialDistribution(Operator):
    ...
    fade_color = BoolProperty(name="Fade colors",
                              default=True,
    ...
    def color_by_cluster(self,bounding_box_centers):
        ...
        for object in bpy.data.objects:
            if object.name.startswith('Soma'):
                ...
                dist = float('inf')
                for i in range(len(bounding_box_centers)):
                    new_dist = self.calc_distance(copy.copy(object.location),
                                                  bounding_box_centers[i])
                    if new_dist < dist:
                        dist = new_dist
                        hue = hues[i]

                ### Calculate falloff for brightness:
                value = 1
                if self.fade_color:
                    falloff = 0.5 / self.radius * dist
                    value -= falloff

                # Make sure brightness doesn't get too low
                value = max(value, .3)
                saturation = 1
    ...
    def find_top_cluster_centers(self,neighbour_list):
        ...
        clusters = sorted(neighbour_list,
                          key=lambda neighbours: neighbours[1],
                          reverse=True)
